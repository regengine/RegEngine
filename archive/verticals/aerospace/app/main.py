...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("aerospace_service_startup")
    yield
    # Shutdown
    logger.info("aerospace_service_shutdown")

app = FastAPI(
    title="Aerospace Compliance Service",
    description="AS9100 and FAA regulatory compliance engine",
    version="1.0.0",
    lifespan=lifespan,
)
...
app.include_router(fai_router, prefix="/api/v1/aerospace")

# Standardized Health & Readiness (Phase 17)
from shared.health import HealthCheck, install_health_router

health = HealthCheck(service_name="aerospace-service")
install_health_router(app, service_name="aerospace-service", health_check=health)
