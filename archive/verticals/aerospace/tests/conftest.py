...
The Aerospace service uses PostgreSQL-specific UUID types which aren't natively
...
# Ensure aerospace service is on the path and clear any conflicting 'app' modules
_aerospace_dir = Path(__file__).resolve().parent.parent
...
for key in _to_remove:
    del sys.modules[key]
sys.path.insert(0, str(_aerospace_dir))
...
try:
    ...
    from services.aerospace.app.models import Base
