...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("manufacturing_service_startup")
    yield
    # Shutdown
    logger.info("manufacturing_service_shutdown")

app = FastAPI(
    title="Manufacturing Compliance Service",
    description="NCR/CAPA tracking and supplier quality management",
    version="1.0.0",
    lifespan=lifespan,
)
...
app.include_router(ncr_router, prefix="/api/v1/manufacturing")
...
health = HealthCheck(service_name="manufacturing-service")
...
install_health_router(app, service_name="manufacturing-service", health_check=health)
