...
async def lifespan(app: FastAPI):
    # Startup
    logger.info("automotive_service_startup")
    yield
    # Shutdown
    logger.info("automotive_service_shutdown")

app = FastAPI(
    title="Automotive Compliance Service",
    description="PPAP vault and safety standard tracking",
    version="1.0.0",
    lifespan=lifespan,
)
...
app.include_router(ppap_router, prefix="/api/v1/automotive")
...
health = HealthCheck(service_name="automotive-service")
...
install_health_router(app, service_name="automotive-service", health_check=health)
