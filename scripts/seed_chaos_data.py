...
# ---------------------------------------------------------------------------
def seed_postgres():
    """Seed PostgreSQL with test API keys and scheduler jobs."""
    try:
        from sqlalchemy import create_engine, text
    except ImportError:
        logger.error("❌ sqlalchemy package not installed. Run: pip install sqlalchemy psycopg2-binary")
        return False
    ...
    with engine.connect() as conn:
        ...
        for i in range(POSTGRES_API_KEY_COUNT):
            conn.execute(
                ...
                {
                    "id": str(uuid4()),
                    "key_hash": f"sha256_chaos_test_{i:04d}",
