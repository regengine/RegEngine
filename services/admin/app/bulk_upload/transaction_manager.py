...
def execute_bulk_commit(
    ...
    *,
    ...
    try:
        ...
        # Enqueue overflow events into the transactional outbox BEFORE commit.
        # Per ``graph_outbox.py`` docstring (lines 14-19), the outbox guarantee
        # is "either both land or neither lands" — so the enqueue must share
        # the transaction with the canonical Postgres write. Post-commit
        # enqueue is what the outbox was designed to eliminate (#1695).
        #
        # An enqueue failure therefore propagates out of this try block and
        # rolls back the whole bulk import. Caller retries with the same
        # payload; the ``bulk_upload:<event_id>`` dedupe_key keeps retries
        # idempotent.
        MAX_SYNC_EVENTS = 100
        ...
        enqueued_count = len(events_to_enqueue)
    ...
    # Graph sync — fast path for the first ``MAX_SYNC_EVENTS``.
    #
    # Writes Neo4j synchronously via :func:`supplier_graph_sync.record_cte_event`.
    # Runs post-commit because the canonical Postgres row must already exist;
    # Neo4j failures here are best-effort and surfaced as ``sync_warnings`` so
    # operators can re-drive out-of-band. The overflow beyond ``MAX_SYNC_EVENTS``
    # has already been atomically enqueued into ``graph_outbox`` above (see
    # the pre-commit loop in the try block — #1695 moved it from here to
    # honor the outbox atomicity contract).
    for event, lot, facility in emitted_event_rows[:MAX_SYNC_EVENTS]:
        try:
