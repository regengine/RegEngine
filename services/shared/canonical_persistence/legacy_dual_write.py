...
To remove: delete this file and remove the two calls in writer.py:
  - legacy_dual_write.dual_write_legacy(...)
  - legacy_dual_write.publish_graph_sync(...)
...
logger = logging.getLogger("canonical-persistence.legacy_dual_write")


# ---------------------------------------------------------------------------
# #1293: Table existence guard
# ---------------------------------------------------------------------------

# Tables this module writes to.  A fresh DB without migrations will be
# missing these; a cryptic IntegrityError or relation-not-found error is
# harder to diagnose than the explicit check below.
_REQUIRED_TABLES = [
...
def validate_tables_exist(engine: Any) -> None:
    """Check that required tables are present; raise a clear error if not.

    Call this at application startup (or in tests that need the legacy path)
    so a fresh DB without migrations fails with an actionable message rather
    than a cryptic SQL error deep in the call stack.

    Args:
        engine: A SQLAlchemy ``Engine`` (or ``Connection``).

    Raises:
        RuntimeError: if any required table is absent.
    """
    inspector = sa_inspect(engine)
    # get_table_names() is schema-aware; pass schema= for each schema we need.
    missing = []
    ...
    if missing:
        raise RuntimeError(
            f"Required tables missing — run Alembic migrations first. "
            f"See alembic/versions/  Missing: {', '.join(missing)}"
        )
