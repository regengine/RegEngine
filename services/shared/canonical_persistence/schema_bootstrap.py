"""
This is NOT an Alembic migration. It bootstraps the schema if tables are absent. Use Alembic for versioned migrations.
...
def dual_write_legacy(session: Session, event: TraceabilityEvent) -> str:
    ...
    # Write KDEs to legacy table.  (#1292 — single bulk INSERT inside the
    # outer transaction; no savepoints.)
    #
    # Previous approach: one nested savepoint per KDE key → 20 SAVEPOINT +
    # 20 RELEASE SAVEPOINT roundtrips for a typical 20-KDE event.
    #
    # New approach: collect all valid KDE rows and execute a single bulk
    # INSERT directly on the outer transaction.  ON CONFLICT DO NOTHING
    # handles duplicates without any savepoint overhead.  Zero SAVEPOINT
    # roundtrips in the common path.
    kde_rows = [
        {
            "tenant_id": str(event.tenant_id),
