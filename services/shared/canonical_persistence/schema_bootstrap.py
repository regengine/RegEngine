...
def dual_write_legacy(session: Session, event: TraceabilityEvent) -> str:
    ...
    # issued 20 roundtrips per event). ON CONFLICT handles duplicate
    # keys; if the whole batch fails we roll back this savepoint and
    # log, but the CTE event row above is preserved.
    kde_rows = [
        {
            "tenant_id": str(event.tenant_id),
