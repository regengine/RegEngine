...
class TestIngestEndpoint:
    ...
    def test_canonical_write_failure_fails_ingest(self, monkeypatch):
        """A broken canonical write must fail the ingest and roll back."""
        session, persistence = _install_shared_stubs(monkeypatch)
    ...
    def test_per_event_fallback_canonical_failure_fails_ingest(self, monkeypatch):
        """Canonical failure in the per-event fallback fails the ingest."""
        session = _FakeSession()
        persistence = _FakePersistence(session, batch_raises=ValueError("batch fail"))
        _install_shared_stubs(monkeypatch, session=session, persistence=persistence)
