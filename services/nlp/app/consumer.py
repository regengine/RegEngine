            ...
            for record in records:
                ...
                with tracer.start_as_current_span(
                    ...
                    try:
                        ...
                        # Guard: empty / whitespace-only text (#1274).
                        # Previously this incremented the success metric and
                        # emitted no review envelope, hiding upstream bugs
                        # (S3 fetch returning empty bytes, text_loader
                        # errors='ignore' on binary, OCR failure, etc.).
                        if not text or not text.strip():
                            logger.warning(
                                "nlp_empty_text_routed_to_review",
                                document_id=doc_id,
                                tenant_id=tenant_id,
                                source_url=source_url,
                                inline=bool(inline_text),
                            )
                            MESSAGES_COUNTER.labels(status="empty_text").inc()
                            # Surface as a review item so operators/reviewers
                            # can diagnose upstream truncation/OCR failure.
                            review_payload = {
                                "tenant_id": str(tenant_id) if tenant_id else None,
                                "document_id": doc_id,
                                "status": "pending",
                                ...
                                "review_reasons": [
                                    {
                                        ...
                                        "description": (
                                            "Document yielded empty or whitespace-only "
                                            "text — possible upstream fetch/OCR failure"
                                        ),
                                        "severity": "error",
                                    }
                                ],
                            ...
                            producer.send(
                                ...
                                + [
                                    ("X-Tenant-ID", str(tenant_id or "").encode()),
                                    ("X-Empty-Text", b"1"),
                                ],
                            )
                            producer.flush(timeout=1.0)
                        ...
                        # Signal: non-empty input but zero entities extracted.
                        # This is a separate regression signal from empty_text
                        # (#1274). Surface it so operators can alert on
                        # extractor drift.
                        if not extractions:
                            MESSAGES_COUNTER.labels(status="zero_entities").inc()
                            logger.warning(
                                "nlp_zero_entities_from_nonempty_text",
                                document_id=doc_id,
                                text_len=len(text),
                            )
