...
The Automotive service uses PostgreSQL-specific UUID types which aren't natively
...
# Ensure automotive service is on the path and clear any conflicting 'app' modules
_automotive_dir = Path(__file__).resolve().parent.parent
...
for key in _to_remove:
    del sys.modules[key]
sys.path.insert(0, str(_automotive_dir))
...
try:
    ...
    from services.automotive.app.models import Base
