            ...
            <div className="max-w-5xl mx-auto space-y-6">
                <div className="flex items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold flex items-center gap-3">
                            <ShieldAlert className="h-6 w-6 text-[var(--re-brand)]" />
                ...
                <Card>
                    ...
                    <CardContent className="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <Input value={scenario} onChange={(e) => setScenario(e.target.value)} className="rounded-xl" />
                        </div>
                        <div>
                            <label className="text-xs font-medium text-muted-foreground mb-1 block">Lots</label>
                            <Input value={lots} onChange={(e) => setLots(e.target.value)} className="rounded-xl" />
                        </div>
                        <div>
                            <label className="text-xs font-medium text-muted-foreground mb-1 block">Date range</label>
                            <Input value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="rounded-xl" />
                        </div>
                        <div>
                            <label className="text-xs font-medium text-muted-foreground mb-1 block">Output package</label>
                            <select className="flex h-10 w-full rounded-xl border border-input bg-background px-3 text-sm">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-xs uppercase tracking-widest text-muted-foreground">Response target</div>
                            <div className="text-lg font-semibold mt-2">Under 24 hours</div>
                            <div className="text-sm text-muted-foreground mt-1">Test weekend, holiday, and supplier-delay conditions.</div>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-xs uppercase tracking-widest text-muted-foreground">Artifacts</div>
                            <div className="text-lg font-semibold mt-2">Package + manifest</div>
                            <div className="text-sm text-muted-foreground mt-1">Each drill records generated artifacts and missing-data warnings.</div>
                        </CardContent>
                    </Card>
                    <Card>
                        <CardContent className="pt-6">
                            <div className="text-xs uppercase tracking-widest text-muted-foreground">Escalation</div>
                            <div className="text-lg font-semibold mt-2">Support path documented</div>
                            <div className="text-sm text-muted-foreground mt-1">Enterprise escalation is contractual; public support does not replace customer-run drills.</div>
                        </CardContent>
                    </Card>
                </div>
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        {runs.map((run) => (
                            <div key={run.id} className="rounded-xl border border-[var(--re-border-default)] p-4">
                                <div className="flex flex-wrap items-center gap-3">
                                    <div className="text-sm font-semibold">{run.scenario}</div>
                                ...
                                <div className="mt-2 flex flex-wrap gap-4 text-sm text-muted-foreground">
                                    <span className="inline-flex items-center gap-1"><TimerReset className="h-4 w-4 text-[var(--re-brand)]" /> {run.elapsed}</span>
                                    <span className="inline-flex items-center gap-1"><FileText className="h-4 w-4 text-[var(--re-brand)]" /> {run.artifacts.join(', ')}</span>
                                </div>
