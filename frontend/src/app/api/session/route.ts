 ...
 *
 * This API route stores the RegEngine API key and admin key in HTTP-only
 * cookies instead of localStorage. The browser cannot read HTTP-only cookies
 * via JavaScript, so XSS attacks cannot exfiltrate these credentials.
 *
 * Usage:
 *   POST /api/session — set credentials (api_key, admin_key, tenant_id)
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { api_key, admin_key, tenant_id } = body;
