...
const API_SERVICES = [
    ...
    },
    {
        name: 'Records API',
        port: 8200,
        baseUrl: '/api/graph/fsma',
        icon: FileText,
        description: 'Traceability record creation, retrieval, and hash verification',
        endpoints: [
            {
                method: 'POST',
                path: '/v1/records',
                description: 'Create a new traceability record (CTE event)',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                body: '{ "cte_type": "RECEIVING", "tlc": "LOT-2026-001", "location": "GLN-123", "quantity": 100, "product_description": "Romaine Lettuce" }',
                response: '{ "id": "uuid", "tlc": "LOT-2026-001", "hash": "sha256:abc...", "created_at": "..." }',
            },
            {
                method: 'GET',
                path: '/v1/records/{id}',
                description: 'Retrieve a traceability record by ID or TLC',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                response: '{ "id": "uuid", "cte_type": "RECEIVING", "tlc": "LOT-2026-001", "hash": "sha256:abc...", "kdes": {...} }',
            },
            {
                method: 'GET',
                path: '/v1/records',
                description: 'Query records with filters (date range, CTE type, product)',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                params: ['start_date=2026-01-01', 'end_date=2026-01-31', 'cte_type=RECEIVING', 'limit=100'],
                response: '{ "records": [...], "total": 150, "next_cursor": "..." }',
            },
            {
                method: 'GET',
                path: '/v1/records/{id}/verify',
                description: 'Verify record hash integrity against immutable ledger',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                response: '{ "valid": true, "hash": "sha256:abc...", "chain_position": 42, "verified_at": "..." }',
            },
            {
                method: 'POST',
                path: '/v1/records/batch',
                description: 'Batch import multiple records (up to 1000)',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                body: '{ "records": [{ "cte_type": "...", "tlc": "...", ... }, ...] }',
                response: '{ "imported": 150, "failed": 2, "errors": [...] }',
            },
        ],
    },
    {
        ...
        endpoints: [
            ...
            {
                method: 'GET',
                path: '/trace/graph/{tlc}',
                description: 'Get full supply chain graph for visualization (D3/Cytoscape)',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                params: ['max_depth=10', 'include_edges=true'],
                response: '{ "nodes": [{ "id": "LOT-001", "type": "lot" }, ...], "edges": [{ "from": "LOT-001", "to": "FAC-001", "cte": "RECEIVING" }], "metadata": {...} }',
            },
            {
                method: 'GET',
                path: '/timeline/{tlc}',
            ...
            },
            {
                method: 'POST',
                path: '/recall/drills',
                description: 'Initiate a mock recall drill for FDA compliance testing',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                body: '{ "type": "forward_trace", "target_tlc": "TLC123", "severity": "class_ii" }',
                response: '{ "drill_id": "uuid", "status": "in_progress", "created_at": "..." }',
            },
            {
                method: 'GET',
                path: '/recall/drills',
                description: 'Get history of mock recall drills',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                params: ['limit=20'],
                response: '{ "drills": [...], "total": 15 }',
            },
            {
                method: 'GET',
                path: '/recall/readiness',
                description: 'Get FSMA 204 recall readiness score and recommendations',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
            ...
            },
            {
                method: 'GET',
                path: '/recall/sla',
                description: 'Get 24-hour SLA compliance metrics',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                response: '{ "average_completion_seconds": 1847, "sla_met_percentage": 100 }',
            },
            {
                method: 'GET',
                path: '/export/fda-spreadsheet',
                description: 'Generate FDA-compliant sortable spreadsheet (CSV)',
                auth: true,
                headers: ['X-RegEngine-API-Key', 'X-Tenant-ID'],
                params: ['tlc=TLC123', 'start_date=2026-01-01', 'end_date=2026-01-31'],
                response: 'CSV file download with all required FSMA 204 columns',
            },
        ],
    },
    {
