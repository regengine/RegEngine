...
export default function FsmaError({
    error,
    reset,
}: {
    ...
    useEffect(() => {
        console.error('FSMA error:', error);
    }, [error]);
            ...
            <div className="max-w-md w-full text-center space-y-6">
                ...
                <div>
                    <h2 className="text-xl font-semibold mb-2">
                        {isBackendError ? 'Backend Unavailable' : 'FSMA Module Error'}
                    </h2>
                    <p className="text-sm text-muted-foreground">
                        {isBackendError
                            ? 'The RegEngine backend is starting up or temporarily unavailable. This usually resolves in 30-60 seconds.'
                            : 'An unexpected error occurred while loading the FSMA module.'}
                    </p>
                </div>
