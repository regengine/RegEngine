...
NOTE: This file lives in tests/integration/ for discoverability alongside
other pipeline tests, but it does NOT require live services.  The autouse
``check_services_health`` fixture from the directory conftest.py is overridden
below so these unit-level tests are never skipped by service-health checks.
"""
...
# ---------------------------------------------------------------------------
# Override the autouse service-health fixture from the directory conftest.
# These tests are fully offline; they must not be skipped because services
# happen to be down.
# ---------------------------------------------------------------------------

@pytest.fixture(autouse=True)
def check_services_health():  # noqa: F811
    """No-op override — all tests in this file are offline/unit-level."""
    return
